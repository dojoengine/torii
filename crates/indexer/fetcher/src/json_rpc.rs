...
impl<P: Provider + Send + Sync + Clone + std::fmt::Debug + 'static> Fetcher<P> {
    ...
    async fn fetch_preconfirmed_block(
        ...
        let mut transactions: IndexMap<Felt, FetchTransaction> = IndexMap::from_iter(preconf_block.transactions.iter().map(|t| {
            (
        ...
        for (contract_address, cursor) in &mut new_cursors.cursors {
            ...
            for t in &preconf_block.transactions {
                ...
                transactions.get_mut(tx_hash).expect("Transaction should exist.").events.extend(events);
